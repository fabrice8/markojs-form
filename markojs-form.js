...
var _allLocalstorage = _interopRequireDefault(require("all-localstorage"));
...
function Patterns() {
  const self = this,
    ...
    url: value => {
      return /^((http(s?)|ftp):\/\/)?[\w-]+(\.[\w-]+)+([\w.,@?^=%&:\/~+#-]*[\w@?^=%&\/~+#-])?/i.test(value);
    },
    email: value => {
      return /^[a-zA-Z0-9.!#$%&'*+/=?^_`{|}~-]+@[a-zA-Z0-9._-]+\.[a-zA-Z]{2,6}$/i.test(value);
    },
    phone: value => {
      return self.number && /\d{7,}|(\d{2,} ) => {3,}/.test(value);
    },
    fullname: value => {
      return /\s+/.test(value);
    },
    password: (value, type) => {
      ...
      let TYPES = {
        ...
        perfect: 4
          ...
          stars = 0,
          status; // Determine the level of password
      ...
      if (stars >= 0 && stars < 2) status = {
        type: 'weak',
        indice: 1
      };else if (stars >= 2 && stars < 4) status = {
        type: 'medium',
        indice: 2
      };else if (stars >= 4 && stars < 6) status = {
        type: 'strong',
        indice: 3
      };else if (stars >= 6) status = {
        type: 'perfect',
        indice: 4
      };
  ...
  function Testor(value, pattern, def) {
    ...
    if (!PredefineList.hasOwnProperty(pattern) && !CustomList.hasOwnProperty(pattern)) return false;
    return PredefineList.hasOwnProperty(pattern) && PredefineList[pattern](value, def) // Pre-defiined pattern test
    || CustomList.hasOwnProperty(pattern) && new RegExp(CustomList[pattern]).test(value); // Custom patterns test
  }
  ...
  this.test = (sequence, value) => {
    // OR composed pattern
    if (/\|/.test(sequence)) {
      const results = sequence.split(/\s*\|\s*/).map(each => {
        return self.test(each, value);
      });
      return results.includes(true);
    } // Single or AND pattern


    if (/\s+/.test(sequence)) return self.multiTest(sequence.split(/\s+/g), value); // pattern of multiple different type of value possible
    else {
  ...
  this.multiTest = (sequenceList, value, extra) => {
    ...
    if (sequenceList.length) for (let o = 0; o < sequenceList.length; o++) {
...
function subSet(obj, link, data) {
  const nsp = link.split('.'),
        toCheck = nsp.shift();
  if (!obj.hasOwnProperty(toCheck) && nsp.length) obj[toCheck] = {};
...
const autofillStore = new _allLocalstorage.default({
  prefix: 'fh--auto-fill',
  encrypt: true
});
...
function FormHandler(options) {
  ...
  this.options = Object.assign({}, defaultOptions, options || {}); // Auto-load saved data of this form

  let savedData;

  if (this.options.autosave) {
    if (!this.options.key) throw new Error('[FormHandler] Undefined Form Key Identifier. Required for Auto-Save');
    if (autofillStore) savedData = autofillStore.get(this.options.key) || {};
  }
  ...
  function applyChange({
    ...
    // Edit the form state
    self.set(name, value);
    self.formError[name] = false; // Test the validation pattern
  ...
  function definePattern({
    ...
    if (!name || !regexp) throw new Error('Invalid Pattern Defined. { <name>, <regexp> } expected'); // Remove regular expression closures "/"
  ...
  this.bind = (component, initForm) => {
    // Binding component
    this.component = component; // Initial form data or schema

    this.initForm = initForm || {}; // populate auto-saved data with initial form data

    this.form = Object.assign({}, this.initForm, savedData); // Set form initial states
    ...
    if (this.options.key) {
      let eForm;
      component.on('mount', () => {
        if (eForm = component.getEl(this.options.key)) eForm.onsubmit = e => e.preventDefault();
      }).on('update', () => {
  ...
  this.set = (name, data) => {
    /\./.test(name) ? subSet(this.form, name, data) // Deep set
    : this.form[name] = data; // shalow set

    this.component.setStateDirty('form', this.form);
    fireEvent('input', [name, data]); // Form auto-saving activated
  ...
  this.fill = data => {
    ...
    fireEvent('fill', [this.form]); // Form auto-saving activated
  ...
  this.reset = () => {
    this.form = {};
    this.formError = {}; // Set form initial states
  ...
  this.define = arg => {
    ...
    if (typeof arg == 'string') {
      ...
      arg.split(/\s+|\|/).map(each => {
        ...
        if (!this.Patterns.has(pattern)) throw new Error('<' + each + '> is not a Predefined Validation Patterm');
      });
      return arg;
    }
    /*----------------------------------------------------------------------*/
    ...
    if (Array.isArray(arg)) arg.map(each => nameSet.push(definePattern(each)));
    /*----------------------------------------------------------------------*/
    // Object or single custom validate pattern: Eg. { name: 'custom', regexp: '/\.+/' }
    else if (typeof arg == 'object') nameSet.push(definePattern(arg)); // Return the concatenated name of defined patterns
  ...
  this.on = (_event, fn) => {
    ...
    if (!PEvents.includes(_event)) return; // Register the listener

    !Events.hasOwnProperty(_event) ? Events[_event] = [fn] // first listener
